...
class TrainLoop:
    def __init__(self, train_platform, model, diffusion, data):
        # self.args = args
        ...
        self.microbatch = config.BATCH_SIZE  # deprecating this option
        self.lr = config.LR
        ...
        if config.DATASET in ['kit', 'humanml','addb'] and config.EVAL_DURING_TRAINING:
            mm_num_samples = 0  # mm is super slow hence we won't run it during training
            mm_num_repeats = 0  # mm is super slow hence we won't run it during training
            gen_loader = get_dataset_loader(name=config.DATASET, batch_size=config.EVAL_BATCH_SIZE, window_size=None,
                                            split=config.EVAL_SPLIT)

            self.eval_gt_data = get_dataset_loader(name=config.DATASET, batch_size=config.EVAL_BATCH_SIZE, window_size=None,
                                                   split=config.EVAL_SPLIT,
                                                   hml_mode='gt')
            self.eval_wrapper = EvaluatorMDMWrapper(config.DATASET, dist_util.dev())
            self.eval_data = {
                'test': lambda: eval_humanml.get_mdm_loader(
                    model, diffusion, config.EVAL_BATCH_SIZE,
                    gen_loader, mm_num_samples, mm_num_repeats, gen_loader.dataset.opt.max_motion_length,
                    config.eval_num_samples, scale=1.,
                )
            }
        self.use_ddp = False
    ...
    def evaluate(self):
        if not config.EVAL_DURING_TRAINING:
            return
        ...
        if self.eval_wrapper is not None:
            ...
            eval_args = SimpleNamespace(num_seeds=self.args.eval_rep_times, num_samples=self.args.eval_num_samples,
                                        batch_size=config.EVAL_BATCH_SIZE, device=self.device, guidance_param = 1,
